                ...
                <div className="bg-white rounded-lg shadow-md p-6 mb-6">
                    <div className="flex justify-between items-start mb-6">
                        <div>
                            ...
                            <p className="text-gray-600 mt-1">Order ID: {order.id.slice(-8)}</p>
                        </div>
                                    ...
                                    return (
                                        <div key={step.status} className="flex-1 relative">
                                            <div className="flex flex-col items-center">
                                                <div className={`w-12 h-12 rounded-full flex items-center justify-center ${isCompleted ? 'bg-primary-600 text-white' : 'bg-gray-200 text-gray-400'
                                                    } ${isCurrent ? 'ring-4 ring-primary-200' : ''}`}>
                                                    <StepIcon size={24} />
                                                </div>
                                                <p className={`mt-2 text-sm font-medium text-center ${isCompleted ? 'text-gray-900' : 'text-gray-400'
                                                    }`}>
                                                    {step.name}
                                                </p>
                                            </div>
                                            {index < steps.length - 1 && (
                                                <div className={`absolute top-6 left-1/2 w-full h-1 ${index < currentStep ? 'bg-primary-600' : 'bg-gray-200'
                                                    }`} style={{ zIndex: -1 }}></div>
                                            )}
                                        </div>
                                    );
                    ...
                    <div className="grid md:grid-cols-2 gap-6 mb-6">
                        <div className="border rounded-lg p-4">
                            ...
                            <p className="text-gray-600 text-sm mt-1">{order.restaurant.address}</p>
                        ...
                        <div className="border rounded-lg p-4">
                            ...
                            <p className="text-gray-600 text-sm mt-1">{order.user.address || 'No address provided'}</p>
                            {order.user.phone && (
                                <p className="text-gray-600 text-sm mt-1 flex items-center">
                                    <Phone size={14} className="mr-1" />
                                    {order.user.phone}
